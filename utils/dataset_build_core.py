...
GENERATED_DIRS = {"images", "labels", "unlabel_images", "unlabel_std", "manifests"}
...
def batch_label_suffixes(batch: Batch) -> set[str]:
...
def read_raw_objects(root: ET.Element, label_path: Path) -> tuple[list[Box], list[dict[str, str]]]:
    boxes: list[Box] = []
...
def override_matches_label(override: dict[str, Any], label_path: Path) -> bool:
    ...
    return any(
        (
            ...
            override.get("label_path_endswith")
            and path_text.endswith(str(override["label_path_endswith"]).replace("\\", "/")),
        )
    )
...
def clean_raw_objects(raw_boxes: list[Box], batch: Batch, label_path: Path, stats: Counter) -> list[Box]:
...
def annotation_size(root: ET.Element, image_path: Path) -> tuple[int, int]:
    ...
    if width <= 0 or height <= 0:
        ...
        width, height, _ = label_convert.image_size(image_path)
        return width, height
    return width, height
...
def output_stem_for_label(label_path: Path, batch: Batch) -> str:
    ...
    for item in batch.rules.get("output_stem_suffixes") or []:
        ...
        if not suffix:
            continue
        if path_contains and path_contains in label_text:
            return f"{stem}{suffix}"
        if path_endswith and label_text.endswith(path_endswith):
            return f"{stem}{suffix}"
    return stem
...
def clean_and_pair(labels: list[tuple[Path, Batch]], images: list[Path], config: dict[str, Any]) -> tuple[list[Sample], list[Path], list[dict[str, str]], Counter]:
    ...
    for label_path, batch in labels:
        ...
        if not boxes:
            ...
            continue
        output_stem = output_stem_for_label(label_path, batch)
        samples.append(
            Sample(
...
def resolve_duplicates(samples: list[Sample], rules: dict[str, Any]) -> tuple[list[Sample], list[list[Sample]], list[dict[str, str]]]:
    ...
    duplicate_groups: list[list[Sample]] = []
    for image_hash, group in sorted(by_hash.items()):
        if len(group) == 1:
            kept.append(group[0])
            continue
        if rules.get("duplicate_same_image_scope") == "same_image_name":
            ...
            for name_group in by_name.values():
                if len(name_group) == 1:
                    kept.append(name_group[0])
                else:
                    duplicate_groups.append(name_group)
            continue
...
def unlabel_output_dir_name(image: Path, config: dict[str, Any]) -> str:
...
def write_outputs(output_dir: Path, samples: list[Sample], unlabels: list[Path], conflicts: list[list[Sample]], issues: list[dict[str, str]], config: dict[str, Any], copy: bool) -> None:
    ...
    for image in unlabels:
        ...
        copy_or_link(image, output_dir / unlabel_dir / unlabel_image_name(image, seen_unlabels), copy)
...
def class_count_row(cls: str, class_instances: Counter, class_files: Counter, class_id: int | None = None) -> dict[str, Any]:
    row: dict[str, Any] = {
        "class_name": cls,
        "instances": class_instances[cls],
        "label_files": class_files[cls],
        "image_files": class_files[cls],
    }
    if class_id is not None:
        row = {"class_id": class_id, **row}
    return row


def write_class_manifests(manifest_dir: Path, classes: list[str], class_instances: Counter, class_files: Counter) -> None:
    fields = ["class_name", "instances", "label_files", "image_files"]
    write_csv(
        manifest_dir / "class_mapping.csv",
        [class_count_row(cls, class_instances, class_files, i) for i, cls in enumerate(classes)],
        ["class_id", *fields],
    )
    (manifest_dir / "classes.txt").write_text("".join(f"{cls}\n" for cls in classes), encoding="utf-8")
    write_csv(
        manifest_dir / "class_counts.csv",
        [class_count_row(cls, class_instances, class_files) for cls in sorted(classes, key=lambda c: (-class_instances[c], c))],
        fields,
    )
...
def unlabel_output_counts(unlabels: list[Path], config: dict[str, Any]) -> Counter:
    counts = Counter()
    for image in unlabels:
        counts[unlabel_output_dir_name(image, config)] += 1
    return counts


def write_summary_manifests(manifest_dir: Path, samples: list[Sample], unlabels: list[Path], issues: list[dict[str, str]], config: dict[str, Any], classes: list[str], class_instances: Counter, split_rows: list[dict[str, Any]], split_meta: dict[str, Any], dist_files: dict[str, defaultdict[str, set[str]]]) -> None:
    train_n = sum(1 for row in split_rows if row["split"] == "train")
    val_n = len(split_rows) - train_n
    unlabel_counts = unlabel_output_counts(unlabels, config)
    summary = {
        "images": len(samples),
        "labels": len(samples),
        ...
        "bad_labels": sum(1 for item in issues if item["issue"] in {"bad_xml", "bad_label"}),
...
def write_manifests(output_dir: Path, samples: list[Sample], unlabels: list[Path], issues: list[dict[str, str]], config: dict[str, Any]) -> None:
    ...
    write_summary_manifests(manifest_dir, samples, unlabels, issues, config, classes, class_instances, split_rows, split_meta, dist_files)
