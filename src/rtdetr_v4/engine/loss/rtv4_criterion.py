...
from ..decoder.dfine_utils import bbox2distance
...
class RTv4Criterion(nn.Module):
    ...
    def __init__(self, \
        ...
        self.fixed_distill_weight = float(self.weight_dict.get('loss_distill', 0.0))
        self.distill_adaptive_params = self._normalize_distill_adaptive_params(distill_adaptive_params)
        if self.has_adaptive_distillation():
            self.current_distill_weight = float(
                self.distill_adaptive_params.get('default_weight', self.fixed_distill_weight)
            )
        else:
            self.current_distill_weight = self.fixed_distill_weight
        self.last_distill_update = None

    def _normalize_distill_adaptive_params(self, params):
        ...
        if delta >= 1.0:
            delta /= 100.0
        default_weight = max(float(params.get('default_weight', self.fixed_distill_weight)), 0.0)
        min_weight = max(float(params.get('min_weight', max(default_weight * 0.25, 0.05))), 0.0)
        max_weight = max(float(params.get('max_weight', max(default_weight * 4.0, default_weight))), min_weight)
        max_update_factor = max(float(params.get('max_update_factor', 2.0)), 1.0)
    ...
    def has_adaptive_distillation(self):
        return bool(self.distill_adaptive_params.get('enabled', False))

    def get_current_distill_weight(self):
        return float(self.current_distill_weight)

    def set_current_distill_weight(self, weight):
        self.current_distill_weight = max(float(weight), 0.0)

    def get_distillation_state(self):
        return {
            'enabled': self.has_adaptive_distillation(),
            'current_weight': self.get_current_distill_weight(),
            'fixed_weight': float(self.fixed_distill_weight),
            'adaptive_params': copy.deepcopy(self.distill_adaptive_params),
        }

    def get_last_distillation_update(self):
        if self.last_distill_update is None:
            return None
        return copy.deepcopy(self.last_distill_update)

    def load_distillation_state(self, state):
        if not isinstance(state, dict):
            return

        current_weight = state.get('current_weight')
        if current_weight is not None:
            self.set_current_distill_weight(current_weight)

    def update_distillation_weight(self, avg_aifi_ratio):
        if not self.has_adaptive_distillation():
            return self.get_current_distill_weight()

        avg_ratio = max(float(avg_aifi_ratio), 0.0)
        rho = float(self.distill_adaptive_params.get('rho', 0.0))
        delta = float(self.distill_adaptive_params.get('delta', 0.0))
        lower = max(rho - delta, 1e-6)
        upper = min(rho + delta, 1.0)

        current_weight = self.get_current_distill_weight()
        if avg_ratio <= 1e-12:
            self.last_distill_update = {
                'avg_aifi_ratio': avg_ratio,
        ...
        target_ratio = upper if avg_ratio < lower else lower
        raw_weight = current_weight * (target_ratio / max(avg_ratio, 1e-6))
        adjusted_weight = raw_weight

        max_update_factor = float(self.distill_adaptive_params.get('max_update_factor', 1.0))
        rate_limited = False
        if max_update_factor > 1.0:
            min_step_weight = current_weight / max_update_factor
            max_step_weight = current_weight * max_update_factor
            adjusted_weight = min(max(adjusted_weight, min_step_weight), max_step_weight)
            rate_limited = abs(adjusted_weight - raw_weight) > 1e-12

        min_weight = float(self.distill_adaptive_params.get('min_weight', 0.0))
        max_weight = float(self.distill_adaptive_params.get('max_weight', max(adjusted_weight, min_weight)))
        clamped_weight = min(max(adjusted_weight, min_weight), max_weight)
        weight_clamped = abs(clamped_weight - adjusted_weight) > 1e-12
        adjusted_weight = clamped_weight
        self.set_current_distill_weight(adjusted_weight)
        self.last_distill_update = {
            'avg_aifi_ratio': avg_ratio,
            'old_weight': current_weight,
            'new_weight': self.get_current_distill_weight(),
            'target_ratio': target_ratio,
            'raw_weight': raw_weight,
            'rate_limited': rate_limited,
            'weight_clamped': weight_clamped,
            'status': 'below_band' if avg_ratio < lower else 'above_band',
        }
        return self.get_current_distill_weight()
    ...
    def loss_distillation(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        if student_feature_map is None or teacher_feature_map is None:
            ...
            for candidate in (student_feature_map, teacher_feature_map, outputs.get('pred_logits'), outputs.get('pred_boxes')):
                if isinstance(candidate, torch.Tensor):
                    fallback_device = candidate.device
                    break
            if fallback_device is None:
                fallback_device = torch.device('cpu')
            return {'loss_distill': torch.tensor(0.0, device=fallback_device, requires_grad=True)}
    ...
    def _get_distillation_weight_for_epoch(self) -> float:
        if self.has_adaptive_distillation():
            return self.get_current_distill_weight()
        return self.fixed_distill_weight
