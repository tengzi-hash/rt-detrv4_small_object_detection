...
class DINOv2TeacherModel(nn.Module):
    """
    Training-only DINOv2 teacher.

    This module lives under ``distill/`` so the student model package stays
    focused on the detector itself. The class still registers into the project
    workspace, which lets the migrated YAML files keep using the same
    ``type: DINOv2TeacherModel`` entry.
    """
